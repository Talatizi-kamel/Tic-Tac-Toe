...
import static ai.MultiLayerPerceptron.load;
    ...
    public class HumaiVsIaController {
        ...
        MultiLayerPerceptron net = load("./resources/models/"+ModeJeuController.NomModel);;
        public double [] etat = new double[9];

        @FXML
        public double [] max(double []tab){
            double max = tab[0];
            double [] indice = new double[2];
        ...
        @FXML
        public int position(double [] etat){
            double [] tab = new double[9];
            tab = net.forwardPropagation(etat);
            double max = max(tab)[1];
            System.out.println(tab);
            int j = 0;
            boolean maxtrouver = false;
            while ((maxtrouver==false)&&(j!=tab.length)){
                if(etat[(int)max(tab)[0]] == 0){
                    j = (int) max(tab)[0];
                    maxtrouver = true;

                }else{
                    tab[(int) max(tab)[0]] = 0.0;
                    j++;
                }
            }
        ...
        private void startNewGame() {
            ...
            net = load("./resources/models/"+ModeJeuController.NomModel);
            setEtat();
            isGameEnds = false;
            isGameWon = false;
            setCurrentTurn();
        ...
        @FXML
        public void clickIa(int i){
            if(!isFirstPlayerTurn) {
                switch (i) {
                    case 0:
                        btn0.fire();
                        btn0.setTextFill(oColor);
                        btn0.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 1:
                        btn1.fire();
                        btn1.setTextFill(oColor);
                        btn1.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 2:
                        btn2.fire();
                        btn2.setTextFill(oColor);
                        btn2.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 3:
                        btn3.fire();
                        btn3.setTextFill(oColor);
                        btn3.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 4:
                        btn4.fire();
                        btn4.setTextFill(oColor);
                        btn4.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 5:
                        btn6.fire();
                        btn5.setTextFill(oColor);
                        btn5.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 6:
                        btn7.fire();
                        btn6.setTextFill(oColor);
                        btn6.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 7:
                        btn7.fire();
                        btn7.setTextFill(oColor);
                        btn7.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                    case 8:
                        btn8.fire();
                        btn8.setTextFill(oColor);
                        btn8.setText("O");
                        isFirstPlayerTurn = !isFirstPlayerTurn;
                        break;
                }
        ...
        private void setWinner() {
            if(isFirstPlayerTurn) {
                ...
                playerTurn.setText("IA a gagner !!!");
            }
            playerTurn.setTextFill(Color.WHITE);
